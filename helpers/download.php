...
function download($arquivo, $nome)
{
	if (isset($arquivo) && file_exists($arquivo)) { // faz o teste se a variavel não esta vazia e se o arquivo realmente existe
		...
		header("Content-Type: " . $tipo, TRUE); // informa o tipo do arquivo ao navegador
		...
		die("<script> window.parent.toast('Arquivo não encontrado: " . $arquivo . "');</script>");
	}
}
